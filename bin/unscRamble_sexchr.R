...
function_unscramble <- function(i, key, out_dir, conv_filelist){
    if (i == 23){
        ...
        fint <- conv_filelist[grep(paste0("chrX", "\\."), conv_filelist)]
    } else {
        # Grep file related to chromosome i
...
if (args[3] %in% c("yes", "YES")){
    res <- lapply(1:23, function_convert, filelist = filelist, out_dir = args[4])
} else {
    res <- lapply(1:22, function_convert, filelist = filelist, out_dir = args[4])
}

# Get all converted files
conv_filelist <- system(paste0("ls ", args[4], "/*log | sed 's/.log//g'"), intern=T)

# Run function to unscramble -- this needs to be checked!!!
if (args[3] %in% c("yes", "YES")){
    res <- lapply(1:23, function_unscramble, key = key, out_dir = args[4], conv_filelist)
} else {
    res <- lapply(1:22, function_unscramble, key = key, out_dir = args[4], conv_filelist)
}
